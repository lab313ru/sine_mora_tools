...
public:
    ...
    void brute_mim_parallel(int depth, int threads) {
        ...
        phase2.brute_parallel(depth - (int)(depth / 2), threads);
    }
    void brute_mim_parallel_hack_for_paths(const unique_ptr<ParallelContext::bits_type>& bits, int depth, int threads) {
        /*if (depth <= 1) {
            brute_parallel(depth); // fallback into simple brute
            return;
        }*/
        ...
        phase1.brute_mim_parallel_internal_step2(bits, depth, threads);
        phase2.known_suffix = "";
        phase2.callback = [this, &looking_hash](const Bruter& B, uint32_t want, uint32_t looking, const string& s) {
            //printf("%08x = sdbm(0, \"%s\")\n", want, s.c_str());
            if (sdbm(0, s) != want)
...
int main(int argc, const char *argv[]) {
    ...
    const int path_depth = 0;
    const int path_threads = 64;
    const string path_known_prefix = "render/stateblocks/";
    if (argc < 5 || argc > 8) {
    ...
    while (true) {
        ...
        if (fscanf(f, "%08X/%08X", &hash_path, &hash) != 2)
            break;
    ...
    if (argc > 7)
        known_suffix = argv[7];
    vector<uint32_t> path_hashes(max_fname_len);
    vector<uint32_t> fname_hashes(path_hashes.size());
    unordered_map<uint32_t, vector<int>> fname_lens_by_path_hash;
    uint32_t fname_ext_hash = hashes[0];
    uint32_t path_fname_hash = hashes_path[fname_ext_hash];
    for (int fname_len = 0; fname_len < path_hashes.size(); ++fname_len) {
        pair<uint32_t, uint32_t> tmp = get_path_and_fname_hashes(path_fname_hash, fname_ext_hash, known_suffix, fname_len);
        path_hashes[fname_len] = tmp.first;
        fname_hashes[fname_len] = tmp.second;
        fname_lens_by_path_hash[tmp.first].push_back(fname_len);
    }
    Bruter b(dict_names, fname_hashes);
    printf("dict_names.size() = %d, dict_paths.size() = %d, hashes.size() = %d\n", (int)dict_names.size(), (int)dict_paths.size(), (int)hashes.size());
    printf("known_prefix = \"%s\"\n", b.known_prefix.c_str());
    printf("known_suffix = \"%s\"\n", b.known_suffix.c_str());
    vector<vector<string>> possible_fname(path_hashes.size());
    mutex possible_fname_mutex;
    b.callback = [&fname_hashes, &possible_fname, &possible_fname_mutex, &known_suffix, fname_ext_hash] (const Bruter &B, uint32_t want, uint32_t looking, const string &s) {
#ifdef _DEBUG
        printf("got %08x = sdbm(0, \"%s\")\n", want, s.c_str());
#endif
        if (s.length() < fname_hashes.size() && fname_hashes[s.length()] == want) {
            lock_guard<mutex> possible_fname_guard(possible_fname_mutex);
            possible_fname[s.length()].push_back(s);
        }
#ifdef _DEBUG
        if (sdbm(0, s) != want)
            printf("error7\n");
        if (sdbm(0, s + known_suffix) != fname_ext_hash)
            printf("error8\n");
#endif
    };
    b.brute_mim_parallel(depth, threads);
    for (int fname_len = 0; fname_len < possible_fname.size(); ++fname_len) {
        vector<string> &vec = possible_fname[fname_len];
        sort(vec.begin(), vec.end());
        auto u = unique(vec.begin(), vec.end());
        vec.resize(u - vec.begin());
    }
    Bruter b_paths(dict_paths, path_hashes);
    b_paths.known_prefix = path_known_prefix;
    b_paths.known_suffix = "";
    b_paths.callback = [&possible_fname, &fname_lens_by_path_hash, path_fname_hash] (const Bruter &B, uint32_t want, uint32_t looking, const string &s) {
        for (int fname_len : fname_lens_by_path_hash[want]) {
            for (auto &fname : possible_fname[fname_len]) {
                printf("!!!! %08x = sdbm(0, \"%s%s\")\n", path_fname_hash, s.c_str(), fname.c_str());
#ifdef _DEBUG
                if (sdbm(0, s + fname) != path_fname_hash)
                    printf("error4: %u != %u\n", sdbm(0, s + fname), path_fname_hash);
#endif
            };
        }
    };
    b_paths.brute_mim_parallel(path_depth, path_threads);
    return 0;
}
