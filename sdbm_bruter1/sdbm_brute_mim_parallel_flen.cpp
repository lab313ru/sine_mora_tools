...
int main(int argc, const char *argv[]) {
    ...
    const string path_known_prefix = "sounds/";
    if (argc < 5 || argc > 8) {
    ...
    /*{
        uint32_t want = path_hashes[10];
        for (int fname_len : fname_lens_by_path_hash[want]) {
            printf("fname_len: %d\n", fname_len);
        }
    }*/
    Bruter b(dict_names, {fname_hashes[12]});
    ...
    for (auto &fname : possible_fname[12]) {
        printf("fname: %s\n", fname.c_str());
    }
    return 0;*/
    ...
    b_paths.known_prefix = known_prefix;
    b_paths.callback = [&possible_fname, &fname_lens_by_path_hash, &fname_hashes, path_fname_hash] (const Bruter &B, uint32_t want, uint32_t looking, const string &s) {
        for (int fname_len : fname_lens_by_path_hash[want]) {
            printf("gotpath %08x = sdbm(0, \"%s\") len=%d fname=%08x\n", want, s.c_str(), fname_len, fname_hashes[fname_len]);
    ...
    b_paths.brute_mim_parallel(depth, threads);
    return 0;
}
